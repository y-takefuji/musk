...
# ── Summary table ────────────────────────────────────────────────────────────
# Helper: format a feature list as a readable semicolon-separated string
def fmt_features(feat_list):
...
summary = pd.DataFrame({
    ...
    "CV10_accuracy": [
        round(rf_top10_accuracy,       4),
        round(xgb_top10_accuracy,      4),
        round(lr_top10_accuracy,       4),
        round(fa_top10_accuracy,       4),
        round(hvgs_top10_accuracy,     4),
        round(spearman_top10_accuracy, 4),
    ],
    "top10_features": [
        fmt_features(rf_top10_features),
        fmt_features(xgb_top10_features),
        fmt_features(lr_top10_features),
        fmt_features(fa_top10_features),
        fmt_features(hvgs_top10_features),
        fmt_features(spearman_top10_features),
    ],
    "top9_features": [
        fmt_features(rf_top9_features),
        fmt_features(xgb_top9_features),
        fmt_features(lr_top9_features),
        fmt_features(fa_top9_features),
        fmt_features(hvgs_top9_features),
        fmt_features(spearman_top9_features),
    ],
})
...
# Save with 4 significant digits preserved for the accuracy column
summary.to_csv("result.csv", index=False, float_format="%.4f")
